...
public class Solution
{
    ...
    public static void cleanAllApartaments(List<Apartament> apartaments)
    {
        ...
        for (int j = 0; j < apartaments.size(); j++)
        {
          if (apartaments.get(j) instanceof Apt1Room )
              ((Apt1Room) apartaments.get(j)).clean1Room();
            else if (apartaments.get(j) instanceof Apt2Room)
              ((Apt2Room)apartaments.get(j)).clean2Rooms();
            else if (apartaments.get(j) instanceof Apt3Room)
              ((Apt3Room)apartaments.get(j)).clean3Rooms();
        }
    }
